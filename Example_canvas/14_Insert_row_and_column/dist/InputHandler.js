import { EditCellCommand } from "./history/commands";
export class InputHandler {
    ...
    handleInputKeyDown(e) {
        ...
        switch (e.key) {
            ...
            case "Tab":
                ...
                break;
            case "Escape":
                this.hideInput(false); // Don't commit on escape
    ...
    commitAndHideInput() {
        if (this.currentInput &&
            ...
            const oldValue = this.grid.getCellValue(this.grid.selectedRow, this.grid.selectedCol);
            // Only create and execute a command if the value actually changed.
            if (newValue !== oldValue) {
                const command = new EditCellCommand(this.grid.selectedRow, this.grid.selectedCol, oldValue, newValue);
    ...
    /**
     * @param {boolean} wasCommitted - This parameter is no longer used but can be kept for API stability.
     */
