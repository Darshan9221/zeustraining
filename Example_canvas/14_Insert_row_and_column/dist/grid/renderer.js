export class Renderer {
    ...
    drawGrid() {
        ...
        const isFullRowSelection = hasSelection && minCol === 1 && maxCol === this.model.cols - 2;
        const isFullColSelection = hasSelection && minRow === 1 && maxRow === this.model.rows - 2;
        ...
        for (let r = Math.max(1, this.model.viewportStartRow - 1); r <= this.model.viewportEndRow; r++) {
            for (let c = Math.max(1, this.model.viewportStartCol - 1); c <= this.model.viewportEndCol; c++) {
        ...
        ctx.beginPath();
        ctx.strokeStyle = "#ddd";
        for (let c = Math.max(1, this.model.viewportStartCol - 1); c <= this.model.viewportEndCol + 1; c++) {
            const x = this.calculator.getColX(c) - this.model.scrollX;
            ctx.moveTo(x + 0.5, 0);
            ctx.lineTo(x + 0.5, this.model.headerHeight);
        }
        for (let r = Math.max(1, this.model.viewportStartRow - 1); r <= this.model.viewportEndRow + 1; r++) {
            const y = this.calculator.getRowY(r) - this.model.scrollY;
            ctx.moveTo(0, y + 0.5);
        ...
        if (hasSelection) {
            ...
            ctx.lineWidth = 1;
            for (let c = Math.max(minCol, this.model.viewportStartCol - 1); c <= Math.min(maxCol + 1, this.model.viewportEndCol + 1); c++) {
                const x = this.calculator.getColX(c) - this.model.scrollX;
                ctx.moveTo(x + 0.5, 0);
                ctx.lineTo(x + 0.5, this.model.headerHeight);
            }
            for (let r = Math.max(minRow, this.model.viewportStartRow - 1); r <= Math.min(maxRow + 1, this.model.viewportEndRow + 1); r++) {
                const y = this.calculator.getRowY(r) - this.model.scrollY;
                ctx.moveTo(0, y + 0.5);
                ctx.lineTo(this.model.headerWidth, y + 0.5);
            }
        ...
        ctx.textAlign = "center";
        ctx.textBaseline = "middle";
        for (let r = Math.max(1, this.model.viewportStartRow - 1); r <= this.model.viewportEndRow + 1 && r < this.model.rows; r++) {
            const screenY = this.calculator.getRowY(r) - this.model.scrollY;
        ...
        for (let c = Math.max(1, this.model.viewportStartCol - 1); c <= this.model.viewportEndCol + 1 && c < this.model.cols; c++) {
            const screenX = this.calculator.getColX(c) - this.model.scrollX;
