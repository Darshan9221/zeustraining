...
export class ContextMenu {
  ...
  constructor(private grid: Grid, private historyManager: HistoryManager) {
  ...
  public show(x: number, y: number, row: number | null, col: number | null) {
    this.menu.innerHTML = "";
    this.menu.style.left = `${x}px`;
    this.menu.style.top = `${y}px`;

    if (col !== null) {
      const insertColBtn = this.createMenuItem("Insert column", () =>
        this.handleInsertColumn(col)
      );
      this.menu.appendChild(insertColBtn);
    }

    if (row !== null) {
      const insertRowBtn = this.createMenuItem("Insert row", () =>
        this.handleInsertRow(row)
      );
  ...
  private createMenuItem(label: string, onClick: () => void): HTMLDivElement {
